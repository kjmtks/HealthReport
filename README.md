...
| `TIMEFRAME` | ※1 | 一日に複数回提出させる場合に設定する |
| `MAIL_SUBJECT` | ※2 | 健康日記アプリから送付されるメールサブジェクト．この文字列をサブジェクトに含むメールを収集対象とする． |
| `OVERRIDE` | `1` | 重複した健康データがある場合に，過去のデータを上書きする場合は何らかの文字列を指定．上書きしない場合は設定しない|
| `SUBDIR` | `/subdir` | サブディレクトリで動作させる場合に指定． |
| `DAYS_TO_SEARCH` | `365` | 設定した場合，その日数分過去の記録のみを検索対象とします．(1とした場合は当日分のみ対象) |
...
同様に，以下を設定することで LDAP を使用した認証機能等が利用できます:
...
加えて以下を設定することで LDAP を使用した観察対象者の登録機能が利用できます:
...
コースの作成・編集画面で，「フィルタボタン」欄に以下の様式で1行につきひとつを記述します．

```
ボタンラベル: 検索の式
```

* 観察対象番号: `student==観察対象者番号`, `student!=観察対象者番号`
...
* 測定年月日: `date==年月日`, `date!=年月日`, `date>=年月日`, `date<=年月日`, `date>年月日`, `date<年月日`
  * 年月日には以下が設定可能:
    * `yyyy/MM/dd`
    * `yyyy/MM/dd+n`: `yyyy/MM/dd` の n 日後の年月日
    * `yyyy/MM/dd-n`: `yyyy/MM/dd` の n 日前の年月日
    * `today`: 当日の年月日
    * `today+n`: 当日の n 日後の年月日
    * `today-n`: 当日の n 日前の年月日
    * `thisweek`: 直前の月曜日の年月日
