...
| `SHOW_UNSUBMITTED_USERS` | `1` | 未提出者のデータをリストに含める場合に何らかの文字列を指定． |
...
*[以下の仕様は変更される予定です](https://github.com/kjmtks/HealthReport/tree/devel-parser)*

「検索の式」には検索条件を表す論理式を記述します．
命題を以下の形式でスペース区切りで複数記述できます．
命題同士は論理積で結合されます．
...
* 測定年月日: `date==年月日`, `date!=年月日`, `date>=年月日`, `date<=年月日`, `date>年月日`, `date<年月日`
  * 年月日には以下が設定可能:
    ...
    * `thisweek`: 直前の月曜日の年月日
    * `thisweek+n`: 直前の月曜日から 7n 日後の年月日
    * `thisweek-n`: 直前の月曜日から 7n 日前の年月日
